...
State MotionPlanner::get_goal_state_in_ego_frame(const State& ego_state, const State& goal_state) {
  ...
  /* Rotate such that the ego state has zero heading/yaw in the new frame.
     We are rotating by -ego_state "yaw" to ensure the ego vehicle's current yaw corresponds to theta = 0 in the new local frame.
  ...
  */
  ...
  goal_state_ego_frame.location.x = cos_theta * goal_state_ego_frame.location.x - sin_theta * goal_state_ego_frame.location.y;
  goal_state_ego_frame.location.y = sin_theta * goal_state_ego_frame.location.x + cos_theta * goal_state_ego_frame.location.y;

  // Compute the goal yaw in the local frame by subtracting off the current ego yaw from the goal waypoint heading/yaw.
  goal_state_ego_frame.rotation.yaw += theta_rad;

  // Ego speed is the same in both coordenates the Z coordinate does not get affected by the rotation.

  // Let's make sure the yaw is within [-180, 180] or [-pi, pi] so the optimizer works.
...
std::vector<State> MotionPlanner::generate_offset_goals(const State& goal_state) {
  // Now we need to gernerate "_num_paths" goals offset from the center goal at a distance "_goal_offset".
  std::vector<State> goals_offset;
  
  // the goals will be aligned on a perpendiclular line to the heading of the main goal. 
  ...
  for (int i = 0; i < _num_paths; ++i) {
    ...
    // TODO-offset goal location: calculate the x and y position of the offset goals using "offset" (calculated above)
    // and knowing that the goals should lie on a perpendicular line to the direction (yaw) of the main goal.
    // You calculated this direction above (yaw_plus_90). HINT: use std::cos(yaw_plus_90) and std::sin(yaw_plus_90)
...
MotionPlanner::transform_spirals_to_global_frame(const std::vector<std::vector<PathPoint>>& spirals, const State& ego_state) {
  ...
  for (auto spiral : spirals) {
    ...
    for (auto path_point : spiral) {
      ...
      new_path_point.x = ego_state.location.x + path_point.x * std::cos(ego_state.rotation.yaw) - path_point.y * std::sin(ego_state.rotation.yaw);
      new_path_point.y = ego_state.location.y + path_point.x * std::sin(ego_state.rotation.yaw) + path_point.y * std::cos(ego_state.rotation.yaw);
...
std::vector<std::vector<PathPoint>> MotionPlanner::generate_spirals(const State& ego_state, const std::vector<State>& goals) {
...
bool MotionPlanner::valid_spiral(const std::vector<PathPoint>& spiral, const State& offset_goal) {
  ...
  // auto dist = utils::magnitude(spiral[spiral.size() - 1].location - offset_goal.location);
  // LOG(INFO) << "Distance from Spiral end to offset_goal: " << dist;
  return (dist < 0.1);
}

float MotionPlanner::calculate_cost(const std::vector<PathPoint>& spiral, const std::vector<State>& obstacles, const State& goal) {
