...
State BehaviorPlannerFSM::get_closest_waypoint_goal(
  ...
  // NOTE 2: GetNextUntilLaneEnd(d) returns a list of waypoints a distance "d" apart. The list goes from the
  // current waypoint to the end of its lane.
...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  // TODO-Lookahead: One way to find a reasonable lookahead distance is to find the distance you will need to 
  // come to a stop while traveling at speed V and using a comfortable deceleration.
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal, bool& is_goal_in_junction, string tl_state) {
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    if (is_goal_in_junction) {
      ...
      // TODO-goal behind the stopping point: put the goal behind the stopping point (i.e the actual goal location)
      // by "_stop_line_buffer". HINTS: remember that we need to go back in the opposite direction of the goal/road, 
      ...
      // TODO-goal speed in nominal state: What should be the goal speed now that we know we are in nominal state and we can continue freely?
    ...
    // TODO-maintain the same goal when in DECEL_TO_STOP state: Make sure the new goal is the same as the previous goal (_goal).
    // That way we keep/maintain the goal at the stop line.
    goal = _goal;
    
    
    // TODO: It turns out that when we teleport, the car is always at speed zero. In this the case, 
    // as soon as we enter the DECEL_TO_STOP state, the condition that we are <= _stop_threshold_speed
    // is ALWAYS true and we move straight to "STOPPED" state. To solve this issue (since we don't
    // have a motion controller yet), you should use "distance" instead of speed. Make sure the distance 
    // to the stopping point is <= P_STOP_THRESHOLD_DISTANCE. Uncomment the line used to calculate the distance
    ...
    if (distance_to_stop_sign <= P_STOP_THRESHOLD_DISTANCE) {
      // TODO-move to STOPPED state: Now that we know we are close or at the stopping point we should change state to "STOPPED"
    ...
    // TODO-maintain the same goal when in STOPPED state: Make sure the new goal is the same as the previous goal.
    // That way we keep/maintain the goal at the stop line. goal = ...;
    goal = _goal;  // Keep previous goal. Stay where you are.
    
    
    long long stopped_secs = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::high_resolution_clock::now() - _start_stop_time).count();
    // LOG(INFO) << "BP- Stopped for " << stopped_secs << " secs";
    
    if (stopped_secs >= _req_stop_time && tl_state.compare("Red") != 0) {
      // TODO-move to FOLLOW_LANE state: What state do we want to move to, when we are "done" at the STOPPED state?
