...
For all trajectories, the required acceleration is given by _a_max (confortable accel).
Look at the structs.h for details on the types of manuevers/states that the behavior planner can be in.
*/
...
std::vector<TrajectoryPoint> VelocityProfileGenerator::generate_trajectory(const std::vector<PathPoint>& spiral, 
  ...
  // If we need to follow the lead vehicle, make sure we decelerate to its speed by the time we reach the time gap point.
  ...
  if (trajectory.size() > 1) {
    ...
    interpolated_state.path_point.x = (trajectory[1].path_point.x - trajectory[0].path_point.x) * 0.1 + trajectory[0].path_point.x;
    interpolated_state.path_point.y = (trajectory[1].path_point.y - trajectory[0].path_point.y) * 0.1 + trajectory[0].path_point.y;
    interpolated_state.path_point.z = (trajectory[1].path_point.z - trajectory[0].path_point.z) * 0.1 + trajectory[0].path_point.z;
...
std::vector<TrajectoryPoint> VelocityProfileGenerator::decelerate_trajectory(const std::vector<PathPoint>& spiral, const double& start_speed) const {
  std::vector<TrajectoryPoint> trajectory;
  ...
  /* If the brake distance exceeds the length of the path, then we cannot perform a smooth deceleration and 
  require a harder deceleration. Build the path up in reverse to ensure we reach zero speed at the required time.
  */
  if (brake_distance + decel_distance > path_length) {
    std::vector<double> speeds;
    auto vf{0.0};
    // Let's add the last point, i.e at the stopping line we should have speed 0.0.
...
std::vector<TrajectoryPoint> VelocityProfileGenerator::nominal_trajectory(const std::vector<PathPoint>& spiral, 
  ...
  for (size_t i = ramp_end_index; i < spiral.size() - 1; ++i) {
    ...
    auto dist = utils::distance(spiral[i], spiral[i + 1]);
    // This should never happen in a "nominal_trajectory", but it's a sanity check
...
double VelocityProfileGenerator::calc_final_speed(const double& v_i, const double& a, const double& d) const {
  double v_f{0.0};
  // TODO-calc final speed: Calculate the final distance. HINT: look at the description of this function. 
